...
const XML_CATALOG = 'cs-app/_176B3E46FC5E6AC9E37206D709D0FAF1';
const STATIC_CATALOG = 'command-catalog.json';
...
function buildCommandCatalog(root = __dirname) {
  const byCommand = new Map();
  const xmlPath = path.join(root, XML_CATALOG);
  const allCommandsPath = path.join(root, 'all-commands.txt');
  ...
  const meta = {
    ...
    sources: {
      ...
      allCommands: fs.existsSync(allCommandsPath) ? 'all-commands.txt' : null,
      auditFile,
    },
...
function candidateCatalogPaths(root = __dirname) {
...
function loadCommandCatalog(root = __dirname) {
